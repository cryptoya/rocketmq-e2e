...
case $1 in
    ...
    pid=`ps ax | grep -i 'org.apache.rocketmq.example.scripts.Producer' |grep java | grep -v grep | awk '{print $1}'`
    if [ -z "$pid" ] ; then
    ...
    pid=`ps ax | grep -i 'org.apache.rocketmq.example.scripts.Consumer' |grep java | grep -v grep | awk '{print $1}'`
    if [ -z "$pid" ] ; then
    ...
    pid=`ps ax | grep -i 'org.apache.rocketmq.example.scripts.TransactionProducer' |grep java | grep -v grep | awk '{print $1}'`
    if [ -z "$pid" ] ; then
    ...
    pid=`ps ax | grep -i 'org.apache.rocketmq.example.scripts.BatchProducer' |grep java | grep -v grep | awk '{print $1}'`
    if [ -z "$pid" ] ; then
